...
export const TwitterView: FC = () => {
  ...
  const handleTabChange = (newTab: string, idx: number) => {
    ...
    if (
      !selectedTweetId[newTab] &&
      tweetsData[newTab] &&
      tweetsData[newTab].length > 0
    ) {
      setSelectedTweetId((prevState) => ({
        ...prevState,
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center">
          {tweets.map((t, j) => (
            <button
              key={`${selectedTab}-${j}`}
              className={`flex flex-col text-center items-center justify-center w-full lg:w-1/2 relative ${
                t._id.toString() !== selectedTweetId[selectedTab]
            ...
            >
              <div className={`${styles["box"]} shadow w-full`}>
                <p className="text-black text-left mr-28">{t["text"]}</p>
                ...
                <a
                  ...
                  className="bg-gray-200 hover:bg-gray-300 active:bg-gray-400 w-24 rounded px-2 py-1 text-xs text-center absolute right-3 top-9"
                >
                  Go to Tweet
                </a>
              </div>
