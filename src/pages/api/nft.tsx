...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const url =
      "https://nft-sales-service.dappradar.com/v3.1/collection?currency=usd&sort=volumeInFiat&order=desc&resultsPerPage=10&page=1&range=day&chainId[]=27";
    ...
    const promises: Promise<NftData | null>[] = data.results.map(
      async (nft, index): Promise<NftData | null> => {
        try {
          ...
          return {
            ...
            url: `https://magiceden.io/marketplace/${
              NFT_NAME_MAP.get(nft.slug) ?? nft.slug
            }`,
